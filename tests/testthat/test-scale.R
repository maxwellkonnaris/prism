...
test_that("prism_scale_log validates and stores settings", {
  s <- prism_scale_log(
    c(1, 2, 3), ci_level = 0.8, estimate_rho = FALSE,
    lower_zero = FALSE, experimental_ci = TRUE
  )
  ...
  expect_error(prism_scale_log(c(1, 2, 3), experimental_ci = NA), "experimental_ci")
  expect_error(prism_scale_log(c(1, 2, 3), ci_level = 0.8), "experimental")
})
...
test_that("validate_scale_input accepts an N x R replicate matrix as-is", {
  log_scale <- matrix(c(-3, -2, -1, 0, 1, 2), nrow = 3L, ncol = 2L)
  m <- .validate_scale_input(log_scale, N = 3L)
  expect_equal(dim(m), c(3L, 2L))
})

test_that("validate_scale_input rejects wrong shape or non-finite values", {
  expect_error(.validate_scale_input(c(1, 2), N = 3L), "N rows")
  expect_error(.validate_scale_input(matrix(1:4, nrow = 2L), N = 3L), "N rows")
  expect_silent(.validate_scale_input(c(1, -2, 0), N = 3L))
  expect_error(.validate_scale_input(c(1, Inf, 3), N = 3L), "finite")
})

test_that("select_scale_log bootstraps and averages technical replicates on the log scale", {
  scale_mat <- matrix(c(-2, -1, 0, 1, 2, 3), nrow = 3L, ncol = 2L)
  set.seed(11L)
  out <- .select_scale_log(scale_mat, sample_index = c(1, 2, 3))
  set.seed(11L)
  expected <- vapply(
    1:3,
    function(i) mean(scale_mat[i, sample.int(2L, 2L, replace = TRUE)]),
    numeric(1)
  )
  expect_equal(out, expected)
})

test_that("select_scale_log returns single-column log-scale values unchanged", {
  scale_mat <- matrix(c(1, 2, 3), nrow = 3L, ncol = 1L)
  out <- .select_scale_log(scale_mat, sample_index = c(2, 1, 3))
  expect_equal(out, c(2, 1, 3))
})
...
test_that("zero-variance scale measurements imply sigma equals zero", {
  X <- matrix(stats::rnorm(6), nrow = 2L)
  expect_error(.feature_correlations(X, rep(1, 3L)), "zero or numerically zero")
  fit <- .estimate_scale_log_bounds(X, rep(1, 3L), ci_level = 0.95)
  expect_identical(fit$sigma_L, 0)
  expect_identical(fit$sigma_U, 0)
  expect_null(fit$rho_L)
  expect_null(fit$rho_U)
  expect_null(fit$rho_witness)
})

test_that("zero-variance log-composition features are an error", {
  X <- rbind(rep(-1, 4L), c(-2, -1, -3, -2))
  expect_error(.feature_correlations(X, c(1, 2, 3, 4)), "feature\\(s\\) 1")
})
