...
.resolve_draw_scale <- function(scale_mode, sample_index, log_props,
  ...
  if (scale_mode$mode == "scale_log") {
    scale_log_s <- scale_mode$scale_log[sample_index]
  } else {
    R <- ncol(scale_mode$scale_mat)
    replicate_index <- if (R == 1L) NULL else sample.int(R, length(sample_index), replace = TRUE)
    scale_log_s <- .select_scale_log(scale_mode$scale_mat, sample_index, replicate_index)
  }
...
#'   `list(mode = "none")`, `list(mode = "scale_log", scale_log = ..., ci_level = ..., estimate_rho = ..., lower_zero = ...)`,
#'   or `list(mode = "scale", scale_mat = ..., ci_level = ..., estimate_rho = ..., lower_zero = ...)`.
...
#' @return A list with `lower`, `upper`, `rel` (D x D x S_eff arrays over
#'   accepted draws), and `diagnostics` (sampling accept/reject bookkeeping).
#' @keywords internal
