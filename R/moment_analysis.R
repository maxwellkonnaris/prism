...
#'
#' This function runs a bootstrapped analysis on the input data matrix \code{Y} and minimizes a squared moment condition to estimate the unknown parameters \code{x, rho1, rho2}.
#'
...
#' @return A list of dataframes containing the results of the analysis including estimated 95% confidence intervals, minimum and maximum values for estimated covariance, and finite sample covariances. \code{final_results_moment} contains the data intended for forest_plot() and \code{all_inner_results_moment} contains the data intended for sigmaplot().
#' @import progress
#' @import progressr
#' @import foreach
#' @import doSNOW
...
#' results <- estimate_covariance_mom(Y)
#' @export

estimate_covariance_mom <- function(Y, alpha = rep(0, nrow(Y)), rhobound = 0.8, S = 1000, variance=FALSE, upperx = 1.0) {
  ...
  moment_condition <- function(params, Yboot, d1, d2, alpha) {
    rho1 <- params[1]
    rho2 <- params[2]
    x <- params[3]
  ...
  objective_function <- function(params, Yboot, d1, d2, alpha) {
    return(moment_condition(params, Yboot, d1, d2, alpha)^2)
  }
    ...
    tryCatch({
      ...
      results_inner <- foreach(s = 1:S, .combine = 'rbind', .packages = c('stats', 'MCMCpack'), .options.snow = opts) %dopar% {
        tryCatch({
          ...
          res_min <- optim(par = c(0, 0, 0.5), fn = objective_function, Yboot = Yboot, d1 = d1, d2 = d2, alpha = alpha, 
          ...
          sigma_min <- moment_condition(res_min$par, Yboot, d1, d2, alpha)
          sigma_max <- moment_condition(res_max$par, Yboot, d1, d2, alpha)
      ...
      # Gather the min and max optimized sigmas and parameters
      minsigma_values <- results_inner$minsigma
      maxsigma_values <- results_inner$maxsigma
      min_rho1_values <- results_inner$min_rho1
      min_rho2_values <- results_inner$min_rho2
      min_x_values <- results_inner$min_x
      max_rho1_values <- results_inner$max_rho1
      max_rho2_values <- results_inner$max_rho2
      max_x_values <- results_inner$max_x
      
      # Sort the results
      sortedmin <- sort(minsigma_values)
      sortedmax <- sort(maxsigma_values)
      
      # Compute the minimum, maximum, and confidence intervals for sigma
      ...
      # Compute the confidence intervals for rho1, rho2, and x
      ci_rho1 <- quantile(min_rho1_values, probs = c(0.025, 0.975), na.rm = TRUE)
      ci_rho2 <- quantile(min_rho2_values, probs = c(0.025, 0.975), na.rm = TRUE)
      ci_x <- quantile(min_x_values, probs = c(0.025, 0.975), na.rm = TRUE)
      ...
      list(
        ...
        results = data.frame(
          ...
          max_rho2 = max_rho2,
          max_x = max_x,
          ci_rho1_lower = ci_rho1[1],
          ci_rho1_upper = ci_rho1[2],
          ci_rho2_lower = ci_rho2[1],
          ci_rho2_upper = ci_rho2[2],
          ci_x_lower = ci_x[1],
          ci_x_upper = ci_x[2],
  ...
  return(list(final_results_moment = final_results, all_inner_results_moment = all_inner_results))
}
