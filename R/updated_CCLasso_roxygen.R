...
#' CCLasso: Correlation Inference for Compositional Data using Lasso
#'
#' Adapted from the CCLasso implementation by Fang Huaying (Peking University).
#' This function performs correlation inference for compositional data using the Lasso method.
#'
#' @param x A matrix of compositional data (n x p), where rows are samples and columns are variables.
#' @param counts A logical value indicating whether the data is a count matrix (default: FALSE).
#' @param pseudo A pseudo-count to add if counts is TRUE (default: 0.5).
#' @param k_cv The number of cross-validation folds (default: 3).
#' @param lam_int Interval for tuning parameter (default: c(1e-4, 1)).
#' @param k_max Maximum iterations for the golden section method (default: 20).
#' @param n_boot Number of bootstrap iterations (default: 20).
#' @return A list containing variance, correlation, p-values, and the selected lambda.
#' @examples
#' # Example usage
